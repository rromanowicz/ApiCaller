package com.rromanowicz.apicaller.test.it

import com.rromanowicz.apicaller.incomming.IncomingRequestLoggingFilter
import com.rromanowicz.apicaller.test.config.SpockIT
import com.rromanowicz.apicaller.unirest.config.LoggingInterceptor
...
@Stepwise
@Title("Unirest")
class UnirestCallerImplIT extends SpockIT {
    ...
    def "GET all 200"() {
        ...
        when:
        def response = request.get("/v2/test")
        ...
        assert incoming.incomingRequest().url() == "/v2/test"
    ...
    def "GET byId 200"() {
        ...
        when:
        def response = request.get("/v2/test/1?param=Test")
        ...
        def incoming = parseIncomingLog(incomingLogs.list.get(0).message)
        assert incoming.incomingRequest().method() == "GET"
        assert incoming.incomingRequest().url() == "/v2/test/1"
    ...
    def "GET byId 404"() {
        ...
        when:
        def response = request.get("/v2/test/9?param=Test")
        ...
        def incoming = parseIncomingLog(incomingLogs.list.get(0).message)
        assert incoming.incomingRequest().method() == "GET"
        assert incoming.incomingRequest().url() == "/v2/test/9"
        assert incoming.incomingRequest().query() == "param=Test"
    ...
    def "POST 201"() {
        ...
        when:
        def response = request.post("/v2/test")
